...
facture facture::chercher(int idfacture){
    ...
    facture F1;
         ...
         checkQuery.prepare("select FACTURE.*,client.nom from FACTURE INNER JOIN COMMANDE on (commande.id_commande=facture.id_commande) INNER JOIN CLIENT ON (client.id_client=commande.id_client) where(facture.nfacture=:idfacture)");
